  ...
  return (
    <section id="problema" className="py-20 md:py-28 bg-white border-b border-slate-200/80">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row items-center justify-center gap-6 md:gap-10 max-w-4xl mx-auto mb-16 text-center md:text-left">
          {/* Avatar ilustrativo de usuario confundido */}
          <div className="relative flex-shrink-0">
            <div className="absolute -inset-2 bg-gradient-to-r from-rose-400 to-amber-300 rounded-full blur-lg opacity-40 animate-pulse-subtle" />
            ...
            <span className="absolute -top-2 -right-2 w-8 h-8 rounded-full bg-rose-500 text-white flex items-center justify-center shadow-lg animate-bounce">
              ❓
            </span>
          </div>
        ...
        {/* Cuadrícula de puntos de dolor (Estilo Claro Premium Moderno) */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {painPoints.map((item, index) => {
            ...
            return (
              <div
                key={index}
                className="relative group rounded-2xl bg-gradient-to-b from-white to-slate-50/80 border border-slate-200/90 p-6 sm:p-7 shadow-md shadow-slate-200/50 hover:shadow-2xl hover:shadow-rose-500/10 hover:border-rose-300 transition-all duration-300 transform hover:-translate-y-2 flex flex-col justify-between overflow-hidden"
              >
                {/* Resplandor decorativo sutil en hover */}
                <div className="absolute -top-12 -right-12 w-32 h-32 bg-rose-500/5 rounded-full blur-2xl group-hover:bg-rose-500/15 transition-all duration-300 pointer-events-none" />

                <div>
                  {/* Encabezado de la tarjeta: Ícono y número de paso */}
                  <div className="flex items-center justify-between mb-5">
                    <div className="w-12 h-12 rounded-xl bg-rose-50 border border-rose-200/80 text-rose-600 flex items-center justify-center shadow-sm group-hover:scale-110 group-hover:bg-rose-600 group-hover:text-white transition-all duration-300">
                      <Icon className="w-6 h-6" />
                    </div>
                    <span className="font-mono text-xs font-extrabold text-rose-700 bg-rose-100/80 border border-rose-200 px-2.5 py-1 rounded-full shadow-2xs">
                      {stepNumber}
                    </span>
                  </div>

                  {/* Título del problema */}
                  <h3 className="text-xl font-extrabold text-slate-900 mb-3 tracking-tight group-hover:text-rose-600 transition-colors">
                    {item.title}
                  </h3>
                ...
                <div className="pt-4 border-t border-rose-100 mt-auto bg-rose-50/70 -mx-6 -mb-6 p-4 rounded-b-2xl group-hover:bg-rose-100/70 transition-colors">
                  <span className="block text-lg font-black text-rose-600 tracking-tight flex items-center gap-1.5">
