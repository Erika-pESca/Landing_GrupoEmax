...
export const ContactoFlotante: React.FC = () => {
  ...
  // El mensaje al lado del chatbot solo aparece por cierto tiempo (7 segundos)
  useEffect(() => {
  ...
  const handleEnviarMensaje = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      // Respuesta predeterminada cuando el usuario saluda ("hola", "buenas", etc.)
      const esSaludo =
        query === "hola" ||
  ...
  return (
    <aside aria-label="Asistente flotante" className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-50 flex flex-col items-end gap-3">
      ...
      {/* 2. Botón de WhatsApp directo (+34 692 42 76 90) */}
      {!isOpen && (
        <a
          href="https://wa.me/34692427690?text=Hola%20Grupo%20EMAX,%20tengo%20una%20duda%20sobre%20mi%20factura%20de%20luz/gas."
          target="_blank"
          ...
          title="Chat de WhatsApp directo (+34 692 42 76 90)"
        >
          <MessageCircle className="w-6 h-6" />
        </a>
      )}

      {/* 3. Fila inferior: Mensaje al lado del Chatbot + Botón Principal de Maxi */}
      <div className="flex items-center gap-3">
        {/* Bocadillo informativo al lado del chatbot (se oculta automáticamente tras 7s) */}
        {!isOpen && showTooltip && (
          <div className="bg-slate-900/95 text-white text-xs py-2 px-3.5 sm:px-4 rounded-2xl shadow-2xl border border-cyan-500/40 max-w-[240px] sm:max-w-xs flex items-center justify-between gap-2.5 animate-in fade-in slide-in-from-right-3 duration-300 backdrop-blur-md">
            <button
              type="button"
              onClick={() => setIsOpen(true)}
              className="flex items-center gap-2 text-left hover:text-cyan-300 transition-colors"
            >
              <div className="w-6 h-6 rounded-full overflow-hidden flex-shrink-0 border border-cyan-400">
                <img
                  src="/imagenes/avatar_chatbot.jpg"
                  alt="Avatar Maxi"
              ...
              <span className="font-medium text-slate-200 text-[11px] sm:text-xs leading-snug">
                💬 ¡Hola! Soy <strong className="text-cyan-300">Maxi</strong>. ¿Dudas con tu factura?
              </span>
            </button>
            <button
              type="button"
        ...
        <button
          ...
          className="relative w-16 h-16 rounded-full bg-gradient-to-tr from-sky-500 via-cyan-500 to-emerald-500 p-[3px] shadow-2xl shadow-cyan-500/40 transition-all transform hover:scale-110 active:scale-95 flex items-center justify-center group flex-shrink-0"
